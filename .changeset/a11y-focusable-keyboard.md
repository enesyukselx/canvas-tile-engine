...
Behavior change: an interactive surface is now a keyboard tab stop by default, and arrow keys pan it. `accessibility.focusable` defaults to `true` whenever any pointer interaction is enabled, so on upgrade every map, board and editor gains a `tabindex` it did not have — tab order shifts, and an app with a global `outline: none` reset would show an invisible stop if the engine did not ship its own `:focus-visible` outline. Set `accessibility: { focusable: false }` to opt out.
...
`preventDefault` runs only for a key the engine consumed. `Tab`, `Escape`, `Home`, `End`, `PageUp` and `PageDown` are never captured, so the surface can never become a keyboard trap, and any modifier combination is ignored so browser and screen-reader shortcuts keep working. Keyboard zoom reports through `onZoom` only, never `onWheel`.

New API: `accessibility.label` / `.description` / `.role` / `.focusable`, `eventHandlers.keyboard`, `engine.setAccessibility(patch)`, and the `AccessibilityRole` / `KeyboardConfig` / `ClickInfo` types. `role` is a narrow union (`"region" | "image" | "application"`) rather than an ARIA passthrough, because widening a union later is free while narrowing a shipped string is a compile break. There is no default accessible name: with no `label`, neither `aria-label` nor `role` is written.
