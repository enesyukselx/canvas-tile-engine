...
- `"preserve-viewport"` rescales `minScale`/`maxScale` together with the base scale, so they act as zoom factors of the configured `scale` at every container width. The wrapper no longer gets CSS `min-width`/`max-width` derived from the scale limits, so the canvas can shrink and grow with any container instead of overflowing narrow layouts.
