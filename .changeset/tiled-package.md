...
- `tiledMapBounds(map)` returns the map's corner-space world extents for wiring the camera: `config.bounds`/`engine.setBounds` (no panning off the map) and `engine.fitBounds` (zoom-to-fit).
