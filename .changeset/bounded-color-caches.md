...
- Both caches are now a fixed-capacity LRU (1024 entries). A hand-authored palette never reaches the bound and stays resident because it is touched every frame; computed one-off colors cost a fixed amount of memory instead of one permanent entry each.
