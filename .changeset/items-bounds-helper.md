...
- New pure helper `itemsBounds(items)`: the world rectangle enclosing a list of items, or `null` when nothing in the list has bounds. Replaces the hand-rolled min/max reduce every "fit to selection" flow needed: `engine.fitBounds(itemsBounds(selected), { paddingPx: 24 })`.
