...
// Conservative max dimension for the offscreen static cache
const MAX_STATIC_CANVAS_DIMENSION = 16384;

/** Factory that creates a fresh offscreen canvas for static caching. */
export type OffscreenCanvasFactory = (width: number, height: number) => Canvas;
 ...
 * Shares the Canvas2D drawing pipeline (culling, spatial index, origin,
 * rotation, static caching) with the browser renderer, retargeted at the
 * `@napi-rs/canvas` context.
 * @internal
 */
export class CanvasDraw implements IDrawAPI<Image> {
    ...
    drawImage(
        items: Array<ImageItem<Image>> | ImageItem<Image>,
        layer: number = 1,
    ): DrawHandle {
        const list = Array.isArray(items) ? items : [items];
    ...
    private addStaticCacheLayer(
        ...
        return this.layers.add(layer, ({ ctx, config, topLeft }) => {
            ...
            // === Bounds Clamping ===
            // Clamp source rect to valid cache bounds and adjust destination rect
            // to only draw the portion that exists in the cache.
     ...
     * Renders all items once to an offscreen canvas, then blits the visible portion each frame.
     * @param items Array of image objects with position and native Image handle
     * @param cacheKey Unique key for this cache (e.g., "terrain-cache")
     * @param layer Layer order
     */
    drawStaticImage(
        items: Array<ImageItem<Image>>,
        cacheKey: string,
        layer: number = 1,
    ): DrawHandle {
        const cache = this.getOrCreateStaticCache(
