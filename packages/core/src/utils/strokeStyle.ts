...
export function resolveLineWidthPx(style: StrokeWidthStyle | undefined, scale: number): number {
    if (style?.lineWidthPx !== undefined) {
        return style.lineWidthPx;
    }
    if (style?.lineWidth !== undefined) {
        return style.lineWidth * scale;
    }
    return 1;
}
...
export function resolveLineDashPx(style: LineDashStyle | undefined, scale: number): number[] | undefined {
    ...
    if (source.some((v) => !Number.isFinite(v) || v < 0)) {
        return undefined;
    }
    ...
    if (pattern.reduce((sum, v) => sum + v, 0) <= 0) {
        return undefined;
    }
    return pattern;
}
...
export function resolveCornerRadiusPx(style: CornerRadiusStyle | undefined, scale: number): number {
    if (style?.cornerRadiusPx !== undefined) {
        return Math.max(0, style.cornerRadiusPx);
    }
    if (style?.cornerRadius !== undefined) {
        return Math.max(0, style.cornerRadius * scale);
    }
    return 0;
}
